...
void HyperCube_Class::check_list() {
    int tag = 0;
    int min_nbr = -1;		// Assumes list contains non-negative integers; 6-21-2017
...
void HyperCube_Class::HyperCube_QuickSort() {
	...
	if (nbr_k > my_id) {
	    ...
	    // ***** Add MPI call here *****
	    MPI_Send(&list[idx] , list_size - idx, MPI_INT, nbr_k, 0, MPI_COMM_WORLD); 
	    ...
	    // ***** Add MPI call here *****
	    MPI_Send(&list[0] , idx, MPI_INT, nbr_k, 0, MPI_COMM_WORLD);
